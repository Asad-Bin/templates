...
point rotate(point p, point c, double rad) {
    p.x -= c.x, p.y -= c.y;
    return point(p.x * cos(rad) - p.y * sin(rad) + c.x, p.x * sin(rad) + p.y * cos(rad) + c.y);
}
bool areParallel(line l1, line l2) { return EQ(l1.a, l2.a) && EQ(l1.b, l2.b); }
bool areSame(line l1, line l2) { return areParallel(l1, l2) && EQ(l1.c, l2.c); }
...
vec perpendicular(vec v) { return vec(-(v.y), v.x); }
...
bool onSegment(point a, point b, point p) { return min(a.x, b.x) <= p.x && p.x <= max(a.x, b.x) && min(a.y, b.y) <= p.y && p.y <= max(a.y, b.y); }
...
bool inPolygon(point poly[], int n, point p) {
    ...
    for (i = 1; i < n; ++i)
            if (collinear(p, poly[i - 1], poly[i]) && onSegment(p, poly[i - 1], poly[i])) return true;
