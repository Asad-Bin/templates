...
struct DSU {
  int p[N];
  void init(int x) { for (int i = 1; i <= x; i++)  p[i] = i; }
...
int query(int x) {
  int r = 2e9 + 10, id = -1;
  for (; x <= n; x += (x & -x)) if (tt[x].val < r) r = tt[x].val, id = tt[x].id;
  return id;
}
void modify(int x, int w, int id) {
  for (; x > 0; x -= (x & -x)) if (tt[x].val > w) tt[x].val = w, tt[x].id = id;
}
int dist(PT &a, PT &b) {
...
void Manhattan() {
  ...
  for (int dir = 1; dir <= 4; ++dir) {
    ...
    for (int i = 1; i <= n; ++i) tt[i].val = 2e9 + 10, tt[i].id = -1;
    for (int i = n; i >= 1; --i) {
      int pos = query(a[i]);
